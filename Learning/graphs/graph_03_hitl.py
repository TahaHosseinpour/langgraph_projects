...
def assemble_outline(state: PresentationState) -> dict:
    ...
    return {
        "slides":   assembled,
        "messages": [AIMessage(content="Outline assembled. Waiting for human review...")],
    }


def human_review(state: PresentationState) -> dict:
    edited = interrupt({
        "design_system": state["design_system"],
        "slides":        state["slides"],
    })
    return {
        "design_system": edited["design_system"],
        "slides":        edited["slides"],
        "approved":      True,
        "messages":      [AIMessage(content="Human review complete. Proceeding to HTML generation...")],
    }
...
def create_graph():
    ...
    builder.add_edge("assemble_outline", "human_review")
    builder.add_edge("human_review",     "generate_html")
