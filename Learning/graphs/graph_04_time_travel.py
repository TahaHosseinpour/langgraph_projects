...
def assemble_outline(state: PresentationState) -> dict:
    # MemorySaver saves a checkpoint here automatically.
    # If validate_html fails, the server replays from this checkpoint via get_state_history().
    ...
    return {
        "slides":   assembled,
        "messages": [AIMessage(content="Outline assembled. [CHECKPOINT] — time travel target is here.")],
    }
...
def validate_html(state: PresentationState) -> dict:
    ...
    if "reveal.js" not in html:
        raise ValueError("Missing reveal.js CDN — time travel back to assemble_outline")
    if "<section" not in html:
        raise ValueError("Missing slide sections — time travel back to assemble_outline")
    if len(html) < 500:
        raise ValueError("HTML too short — time travel back to assemble_outline")
    return {}


# ── Graph ──────────────────────────────────────────────────────────────────

# Module-level so history persists across calls in the same server session.
